...
function move() {
    ...
    if (direction == 'x-') {
        ...
        if (snake_coords[1] == (FIELD_SIZE_Y - FIELD_SIZE_Y)) {
...
function createBlock() {
    ...
    while (!blockCreated) {
        let block_x = Math.floor(Math.random() * FIELD_SIZE_X);
        let block_y = Math.floor(Math.random() * FIELD_SIZE_Y);

        let block_cell = document.getElementsByClassName('cell-' + block_y + '-' + block_x)[0];
        let block_cell_classes = block_cell.getAttribute('class').split(' ');

        if (!block_cell_classes.includes('snake-unit')) {
            let classes = '';
            for (let i= 0; i < block_cell_classes.length; i++) {
